    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">Audit trail of completed transformations, outputs, and tokens consumed.</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-white/5 text-slate-300">
                {filteredJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-4 px-2 text-cyan-400 font-mono font-medium">{job.tokens}</td>
                    <td className="py-4 px-2 text-slate-400">{job.date}</td>
